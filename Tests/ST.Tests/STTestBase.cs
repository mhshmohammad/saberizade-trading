...
namespace ST.Tests
{
    public abstract class STTestBase : AbpIntegratedTestBase<STTestModule>
    {
        ...
        protected void UsingDbContext(Action<STDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<STDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<STDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<STDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<STDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<STDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<STDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<STDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<STDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<STDbContext, Task<T>> func)
        {
            T result;
